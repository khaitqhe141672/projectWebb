...
public class UserOrderServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<Order> ls = new OrderDao().getOneAccount(ac.getId());
            
            String sorderId = request.getParameter("id");
            int orderId = NumberUtil.getNumber(sorderId, 0);
            
            if(orderId != 0){
                List<OrderDetail> lsOrder = new OrderDetailDao().getOneAccount(orderId);
                request.setAttribute("lsOrderDetails", ls);
            } else {
